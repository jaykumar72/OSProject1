 #include<stdio.h> 
#include<stdbool.h> 
//->Below is the waiting time function in which we are calculating the waiting time of each process. 
...
->Below is the function of TurnAroundTime in which we will calculate turnaround time of each process void TurnAroundTime(int process[], int n,int burst[], int wt[], int temp[]) 
{ 
    int i;     for (i = 1; i <=n ; i++)         temp[i] = burst[i] + wt[i]; 
} 
 
 
///->Below is the AverageTime function, in which we will call WaitingTime(), 
//TurnAroundTime() functions, and displaycode of the burst time, processes, arrival time, waiting time, turn around time. We will also calculate the total time spent to handle student and faculty queries. 
...
//->Below is the main function, in which we will initialize the QuantumTime, ask the user to enter no.of processes, arriaval time, burst time. We call AvgTime function in this. We call all the functions in it and it prints the required output on desktop.  
int main() {     int y; 
